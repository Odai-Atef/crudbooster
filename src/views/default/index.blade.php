        ...
        <div class="box-header">
            ...
            <div class="box-tools pull-{{ cbLang('right') }}"
                ...
                @if($button_filter)
                    @if($table=="projects")
                        <?php
                        $selected_user = request('filter_column')['cms_users.name']['value'];
                        $users = \Illuminate\Support\Facades\DB::table("cms_users")->get();
                        ?>
                        <select style="margin-bottom: 10px" onchange="applyRecommendedByFilter()" id="recommendedBy"
                                class="form-control" required="">
                            <option value="">مرشح بواسطه</option>
                            @foreach($users as $user)
                                <option
                                    {{($selected_user==$user->name?"selected":"")}} value="{{$user->name}}">{{$user->name}}</option>
                            @endforeach
                        </select>
                        <script type="text/javascript">
                            function applyRecommendedByFilter() {

                                $("[name='filter_column[projects.is_recommended][type]']").val("=");
                                $("[name='filter_column[projects.is_recommended][value]']").val("1");
                                $("[name='filter_column[projects.is_recommended][value]']").removeAttr("disabled");
                                $("[name='filter_column[cms_users.name][type]']").val("like");
                                $("[name='filter_column[cms_users.name][value]']").val($("#recommendedBy").val());
                                $("[name='filter_column[cms_users.name][value]']").removeAttr("disabled");

                                $("#advanced_filter_modal").find("form").submit();
                            }
                        </script>
                    @endif
